        ...
        if ($this->securityContext->isGranted('ROLE_CONTACTO')) {
            ...
            if (!$this->securityContext->isGranted('ROLE_OPERA_HOTEL')) {
                ...
                $intersection = $this->entity_manager->getRepository('FDataTransactionsBundle:Transaction')->createQueryBuilder('t')
                    ...
                    ->setParameter(2, $tags)
                    ->setParameter(3, $this->http_client->getStartDate())
                ...
                $othersTag = ['index' => [], 'tag' => []];
                foreach ($results as $k => $otherResult) {
                    if('Pre-Authorization' === $otherResult[6]) {
                        $othersTag['tag'][] = $otherResult[0];
                        $othersTag['index'][] = $k;
                    }
                }

                if($othersTag) {
                    var_dump($filteredResults, $othersTag);die;
                    $othersTransactions = $this->entity_manager->getRepository('FDataTransactionsBundle:Transaction')->createQueryBuilder('t')
                        ->select('t.usuario, t.transactionTag')
                        ->where('t.transactionTag IN (?1)')
                        ->setParameter(1, $othersTag['tag'])
                        ->getQuery()->getArrayResult();

                    foreach($othersTransactions as $ot) {
                        $filteredResults[] = $results[array_search($ot['transactionTag'], $othersTag['index'])];
                    }
